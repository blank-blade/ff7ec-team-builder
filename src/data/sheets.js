...
export function sheetCsvUrl(sheetUrlOrId, sheetName = "Equipments") {
	...
	if (!id)
		throw new Error(
			"Could not find a Google Spreadsheet ID in the provided URL.",
		);
...
export async function fetchSheetCsv(sheetUrlOrId, sheetName = "Equipments") {
	...
	if (!response.ok) {
		throw new Error(
			`Failed to fetch ${sheetName} as CSV: HTTP ${response.status}. Make sure the copied sheet is public/viewable.`,
		);
	}
...
export async function fetchSampleTsv() {
	...
	if (!response.ok)
		throw new Error(
			`Failed to load bundled sample TSV: HTTP ${response.status}`,
		);
	return await response.text();
}
...
export async function fetchSamplePresetsTsv() {
	...
	if (!response.ok)
		throw new Error(
			`Failed to load bundled presets TSV: HTTP ${response.status}`,
		);
	return await response.text();
}
...
export function validateEquipmentGrid(rows) {
	...
	if (header.length < required.length)
		warnings.push(
			`Expected at least ${required.length} columns, found ${header.length}.`,
		);
	const widthIssues = rows
		.slice(1)
		.reduce((count, row) => count + (row.length < required.length ? 1 : 0), 0);
	if (widthIssues)
		warnings.push(
			`${widthIssues} row(s) have fewer than ${required.length} columns.`,
		);
...
export function validatePresetGrid(rows) {
	...
	for (const col of required) {
		if (!normalized.includes(col))
			warnings.push(`Missing required preset column: ${col}`);
	}
