...
import { buildEffectDefs, EFFECT_DOMAIN, EFFECT_KIND } from "./ui/effects.js";
import {
	normalizeToken,
	parsePresetRows,
	QUICK_PRESETS,
...
const state = {
	...
	damageObjective: "anchor",
	damageWindow: "sustained",
	attackValue: 1000,
	enemyDefense: 100,
	stanceBonus: 50,
	basePotencyBonus: 0,
...
function effectOptionsForRecommendation() {
	return {
		...
		damageObjective: state.damageObjective,
		damageWindow: state.damageWindow,
		attackValue: state.attackValue,
		enemyDefense: state.enemyDefense,
		stanceBonus: state.stanceBonus,
		basePotencyBonus: state.basePotencyBonus,
	};
}
...
function highlightEquipment(effectKey) {
	...
	targets.forEach((el) => {
		...
		const match =
			Boolean(index) &&
			(index.includes(effectKey) || effectKey.includes(index));
